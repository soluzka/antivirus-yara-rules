...
# process_raw_features() methods are unchanged (they only touch the JSON
# dicts already present in the EMBER dataset, never lief itself), so training
# is unaffected. Only the inference-time raw_features() methods (which parse
# a live PE file with lief) needed patching:
#   - lief.PE.parse() now takes bytes directly and returns None on failure
#     instead of raising bad_format/bad_file/pe_error/etc (those exception
#     classes no longer exist at the top level of the lief module).
#   - np.int was removed in numpy >= 1.24; replaced with int.
#   - lief.not_found no longer exists; section_from_rva() now just returns
#     None on a bad RVA, so we check for that directly.
#   - has_signatures (plural) is the current attribute name.
# One known, accepted imprecision: optional_header.dll_characteristics_lists
# returns raw ints in this lief version rather than named enum members (e.g.
# "32" instead of "HIGH_ENTROPY_VA"), so the 10-dim hashed slot for that one
# sub-feature won't line up with what the original 2018 dataset would have
# hashed. That's a small fraction of the 2381-dim header block, not the
# overall feature set, so it's left as a documented limitation rather than
# something worth deeper lief-version archaeology to fix.
...
class SectionInfo(FeatureType):
    ...
    def process_raw_features(self, raw_obj):
        ...
        # NOTE: raw_obj['entry'] is a single string (the section name); it must be
        # wrapped in its own list so FeatureHasher treats it as one sample containing
        # one string token, rather than (as newer sklearn now explicitly rejects)
        # iterating over its individual characters.
    ...
    def __init__(self, feature_version=2, print_feature_warning=True, features_file=''):
        ...
        if feature_version == 1:
            ...
            if print_feature_warning:
                print("WARNING: this feature extractor has been patched to work with modern lief/numpy; "
                      "see the module docstring/comments in ember_vendor/features.py for the specific "
                      "compatibility caveats versus the original 2018 EMBER feature computation.")
        else:
            raise Exception(f"EMBER feature version must be 1 or 2. Not {feature_version}")
        self.dim = sum([fe.dim for fe in self.features])

    def raw_features(self, bytez):
        # Modern lief.PE.parse() takes bytes directly and returns None on a
        # parse failure rather than raising one of the (now removed)
        # lief.bad_format/bad_file/pe_error/parser_error/read_out_of_bound
        # exceptions, so there's no try/except needed here anymore.
