...
def _is_path_under_any(path, bases):
    """Return True if `path` resolves to a location under one of the `bases` directories."""
    try:
...
# Note: SECRET_KEY should be set in environment, not auto-generated
...
# service.cache is normally created by the installer/launcher. If it's
# missing (dev checkout, manual run), fall back to the project .env so
# CLOUD_API_KEY and friends are still available and the cloud agent can
# register. Existing environment variables always win (no override).
_fallback_env = os.path.join(basedir, '.env')
...
class ThreatDetectionModel:
    ...
    def scan_file(self, file_path):
        ...
        Only high/critical severity matches are treated as confirmed threats.
        Low and medium severity matches are returned as 'suspicious' so the
        UI can surface them for review without triggering auto-quarantine.
        """
        try:
            # Import the scanner from the security module
            from security.yara_scanner import scan_file_with_yara, get_match_severity, has_critical_yara_match, get_highest_severity, _rank_of
            ...
            if matches and len(matches) > 0:
                ...
                rule_labels = [m['rule'] for m in match_info]
                # Only high/critical matches are treated as confirmed threats.
                # Medium matches are suspicious (shown for review, not auto-quarantined).
                if _rank_of(highest) >= _rank_of('high'):
                    self.logger.warning(f"YARA threat confirmed in {file_path} (severity: {highest}): {rule_labels}")
                    return {'threat': True, 'matches': match_info, 'severity': highest}
                else:
                    self.logger.info(f"YARA low/medium match in {file_path} (severity: {highest}): {rule_labels}")
                    return {'threat': False, 'suspicious': True, 'matches': match_info, 'severity': highest}
...
def _get_best_ml_score(file_path):
    ...
    try:
        ...
        # Basic SSRF guard: require a plain http(s) license-server URL with a host
        # and no embedded credentials before POSTing scan metadata.
...
from safe_downloader import extract_archive, _is_safe_url, download_and_scan
    ...
    try:
        ...
        for download_info in downloads:
            try:
                ...
                # Resolve the path and enforce that the destination stays inside
                # one of the configured monitored folders (prevents traversal like
                # C:\monitored\..\windows\evil.dll).
                original_path = os.path.abspath(original_path)
                if not _is_path_under_any(original_path, monitored_folders):
    ...
    while True:
        try:
            ...
            if not DOWNLOAD_QUEUE.empty():
                ...
                # Validate the remote URL to prevent SSRF
                if not _is_safe_url(url):
                    logging.warning(f"Blocked unsafe download URL in queue: {url}")
                    continue
                ...
                # Restrict final output to the configured encrypted folder or the
                # project/runtime directory so a queued request cannot write elsewhere.
                allowed_bases = [ENCRYPTED_FOLDER] if 'ENCRYPTED_FOLDER' in globals() else []
                if not allowed_bases:
                    allowed_bases = [os.path.dirname(os.path.abspath(__file__))]
                if not _is_path_under_any(encrypted_path, allowed_bases):
                    logging.warning(f"Blocked download output outside allowed base: {encrypted_path}")
                    continue

                try:
                    result = download_and_scan(url, encrypted_path)
                    if result.get('ok'):
                        logging.info(f"File downloaded, scanned, and encrypted to {encrypted_path}")
                    else:
                        logging.warning(f"Download rejected: {result.get('error')}")
                except Exception as e:
...
if __name__ == '__main__':
    def _find_login_exe():
        for root in (
            ...
            if not root:
                continue
            for name in ('Isolation Bytes', 'Antivirus Server'):
                path = os.path.join(root, name, 'AntivirusServerLogin.exe')
                if os.path.exists(path):
                    return path
                path = os.path.join(root, name, 'IsolationBytesLogin.exe')
                if os.path.exists(path):
                    return path
    ...
    try:
        if os.path.exists(login_path) and subprocess.run([login_path, '--verify'], timeout=30,  # nosec B603
                                                          creationflags=0x08000000).returncode == 0:
            pass
        else:
            print('Activation required. Opening Isolation Bytes Login...')
            if os.path.exists(login_path):
                subprocess.Popen([login_path], creationflags=subprocess.CREATE_NEW_CONSOLE)  # nosec B603
            else:
...
app.secret_key = os.environ.get('CLOUD_SECRET_KEY') or os.environ.get('SECRET_KEY') or secrets.token_hex(32)
...
def safe_download():
    """Download a user-supplied URL, scan it, and store the encrypted result."""
    url = request.values.get('url')
    if not url:
        return render_template('safe_download.html', error='No URL provided')

    if not _is_safe_url(url):
        return render_template('safe_download.html', error='Unsafe or invalid URL')

    # Derive a safe filename from the URL; prevent path traversal/injection.
    parsed = urlparse(url)
    filename = secure_filename(os.path.basename(parsed.path) or 'downloaded_file')
    if not filename or filename in ('.', '..'):
        filename = 'downloaded_file'
    encrypted_name = filename + '.encrypted'
    encrypted_path = os.path.abspath(os.path.join(ENCRYPTED_FOLDER, encrypted_name))
    if not _is_path_under_any(encrypted_path, [ENCRYPTED_FOLDER]):
        return render_template('safe_download.html', error='Invalid output path')

    try:
        os.makedirs(ENCRYPTED_FOLDER, exist_ok=True)
        result = download_and_scan(url, encrypted_path)
        if not result.get('ok'):
            return render_template('safe_download.html', error=f"Download rejected: {result.get('error', 'unknown')}")
        ...
        logging.error(f"Safe download failed for {url}: {e}")
        return render_template('safe_download.html', error=f'Failed to download file: {e}')
    ...
    for root, _, files in os.walk(folder):
        for filename in files:
            ...
            try:
                ...
                if platform.system() == "Windows" and os.path.isfile(filepath):
                    try:
                        # Escape single quotes inside a PowerShell single-quoted string,
                        # then run via -EncodedCommand so the variable never appears in the
                        # process command line as user-supplied text.
...
limiter = Limiter(
    key_func=get_remote_address,
    storage_uri=storage_uri,
    default_limits=[f"{rate_limit_15min} per 15 minutes"],
...
def ratelimit_handler(e):
    if not request.path.startswith('/api/') and request.accept_mimetypes.best != 'application/json':
...
def main():
    try:
        ...
        # Start the cloud agent — reports heartbeats and scans to isolation-bytes.com
        try:
            from security.local_agent import start_local_agent
            _cloud_api_key = os.environ.get('CLOUD_API_KEY', '')
            _cloud_server_url = os.environ.get('LICENSE_SERVER', 'https://isolation-bytes.com')
            if _cloud_api_key:
                start_local_agent(server_url=_cloud_server_url, api_key=_cloud_api_key)
                logging.info("Cloud agent started — reporting to " + _cloud_server_url)
        except Exception as e:
            logging.warning(f"Could not start cloud agent: {e}")
